...
describe('NewRepositoryPage', () => {
  ...
  it('marks onboarding as completed after closing webhook modal in onboarding flow', async () => {
    server.use(
      http.get('/api/auth/me', () =>
        HttpResponse.json({
          id: 'user-1',
      ...
      http.get('/api/me/github-repos', () => HttpResponse.json(['team/docs-ru', 'team/docs-en'])),
      http.post('/api/projects', async ({ request }) => {
        const body = (await request.json()) as {
          name: string
          source_repo: string
    ...
    renderWithProviders(
      <MemoryRouter initialEntries={['/repositories/new?onboarding=1']}>
        <NewRepositoryPage />
      </MemoryRouter>,
      { store },
    )
    ...
    expect(readOnboardingStatus('user-1')).toBe('completed')
  })
})
